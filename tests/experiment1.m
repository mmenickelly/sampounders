function experiment1(test_function, data_type, solver, macro_seed, micro_seed, batchsizes, num_epochs, m)


% PERFORMS BASIC TEST RUNS. 
...
%% First run the solver using uniform generation of I_k, J_k
for k = 1:num_batchsizes
    b = batchsizes(k); 
    rng(micro_seed); % Controls randomness of run (as opposed to generation of problem data) 
    if strcmp(solver,'fo')
        [X,~,~,xkin,Eval] = sam_fo(func,X0,n,npmax,nfmax,gtol,delta,nfs,m,F0,xind,Low,Upp,printf,b,'uniform',lipY);
    elseif strcmp(solver,'pounders')
        [X,~,~,xkin,Eval] = sam_pounders(func,X0,n,npmax,nfmax,gtol,delta,nfs,m,F0,xind,Low,Upp,printf,b,'uniform',lipY);
    else
        error('solver must be set to fo or pounders.');
    end
    ...
    for kk = 1:length(gradcount)
        ...
        Hf(k,prev + 1:gradcount(kk)) = fvals(kk)*ones(1,gradcount(kk)-prev);
        Hg(k,prev + 1:gradcount(kk)) = gvals(kk)*ones(1,gradcount(kk)-prev);
        prev = gradcount(kk); 
    end
    Evals{k} = Eval;
end

%% Now run the solver using adaptive generation of I_k, J_k
for k = 1:(num_batchsizes - 1)
    ...
    if strcmp(solver,'fo')
        ...
        [X,~,~,xkin,Eval] = sam_pounders(func,X0,n,npmax,nfmax,gtol,delta,nfs,m,F0,xind,Low,Upp,printf,b,'adaptive',lipY);
    end
...
filename = strcat('results/',test_function,'_',data_type,'_',solver,'_',num2str(macro_seed),'_',num2str(micro_seed),'_',num2str(m),'.mat');
