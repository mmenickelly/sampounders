% formquad_indep.m, Version 0.1, Modified 3/2/10
% Stefan Wild and Jorge More', Argonne National Laboratory.
% 
%  [Mdir,np,valid,G,H,Mind,C] = formquad_indep(X,F,delta,Xk,npmax,Pars,vf)
%
% Computes the parameters for m quadratics %!! Check delta scaling
%       Q_i(x) = C(i) + G(:,i)'*(x-Xk) + 
%                0.5*(x-Xk)'*H(:,:,i)*(x-Xk),  i=1:m
...
%
%!! New: This routine does not assume that Xk has been evaluated
% It does assume that all m component functions passed have been evaluated
% at the points in X.
%
% --INPUTS-----------------------------------------------------------------
...
% Xk      [dbl] [nf-by-n] X location of the current center
...
%
% --OUTPUTS----------------------------------------------------------------
% Mdir    [dbl] [(n-np+1)-by-n]  Unit directions to improve model(s)
...
%
function [Mdir,np,valid,G,H,Mind,C] = formquad_indep(X,F,delta,Xk,npmax,Pars,vf)
...
%!! Warning: For now we *always* interpolate the closest point whenever
%formquad_indep is called. This is dangerous if you call the routine for
%sets of points far away Xk.
[minNd,kin]=min(Nd); % Find closest point
if minNd > 0 % Need to renormalize D
    disp('Warning: Xk not evaluated')
    for i=1:nf
        D(i,:) = D(i,:) + (Xk-X(kin,:))/delta;
    end
end
Mind = kin; % Indices of model interpolation points
valid = false;
...
H=H/delta^2;
G=G/delta;
%C=F(kin,:); %! Since interpolating at X(kin,:)

%!! Now need to undo shift so that everything is relative to Xk instead of X(kin,:)
if minNd > 0 % Need to renormalize D
    % in saga_pounders, something is very wrong if this is triggered
    disp('Warning: Xk not evaluated')
    D = Xk-X(kin,:);
    for i=1:m
        C(i) = C(i) + D*G(:,i) + 0.5*D*H(:,:,i)*D';
        G(:,i) = G(:,i) + H(:,:,i)*D';
    end
end
